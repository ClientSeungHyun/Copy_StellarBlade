...
void UTargetingComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (ITargetingInterface* Targeting = Cast<ITargetingInterface>(LockedTargetActor))
	{
...
void UTargetingComponent::SwitchingLockedOnActor(ESwitchingDirection InDirection)
{
	if (::IsValid(LockedTargetActor))
	{
		if (ITargetingInterface* Targeting = Cast<ITargetingInterface>(LockedTargetActor))
		{
			...
			{
				if (ITargetingInterface* NewTargeting = Cast<ITargetingInterface>(TargetActor))
				{
...
void UTargetingComponent::FindTargets(TArray<AActor*>& OutTargetingActors) const
{
	...
	if (bHit)
	{
		for (const FHitResult& HitResult : OutHits)
		{
			...
			if (ITargetingInterface* Targeting = Cast<ITargetingInterface>(HitActor))
			{
